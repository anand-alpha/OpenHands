...
class MCPHealthChecker:
    ...
    def check_prerequisites(self) -> bool:
        ...
        if missing:
            print_formatted_text(HTML('<ansiyellow>💡 Missing prerequisites:</ansiyellow>'))
            for name in missing:
                print_formatted_text(HTML(f'  • {name}'))
            print_formatted_text(HTML('<ansiyellow>Install Node.js from: https://nodejs.org/</ansiyellow>'))
            return False
    ...
    def fix_common_issues(self):
        ...
        print_formatted_text(HTML('<ansiblue>🔧 Common MCP Server Issues & Fixes</ansiblue>'))
        print()
        ...
        for fix in fixes:
            print_formatted_text(HTML(f'<ansiyellow>Issue:</ansiyellow> {fix["issue"]}'))
    ...
    def create_minimal_config(self):
        ...
        print_formatted_text(HTML('<ansiblue>🔧 Creating minimal working configuration...</ansiblue>'))
        ...
        print_formatted_text(HTML('<ansiyellow>Created minimal config with echo test server</ansiyellow>'))
        print_formatted_text(HTML('<ansigreen>This should work without any dependencies</ansigreen>'))
        
        if confirm('Save minimal configuration?', default=True):
            self.editor.save_config()
            print_formatted_text(HTML('<ansigreen>✅ Minimal configuration saved</ansigreen>'))
    
    def run_health_check(self):
        """Run complete health check"""
        print_formatted_text(HTML('<ansiblue>🏥 MCP Health Check</ansiblue>'))
        print_formatted_text(HTML('<ansiyellow>Checking your MCP server configuration...</ansiyellow>'))
        print()
        
        # Check prerequisites
        if not self.check_prerequisites():
            print()
            print_formatted_text(HTML('<ansired>❌ Prerequisites missing - install Node.js first</ansired>'))
            return False
        
        print()
        
        # Validate config
        print_formatted_text(HTML('<ansiblue>🔍 Validating configuration...</ansiblue>'))
        ...
        print_formatted_text(HTML('<ansiblue>🔍 Testing server connections...</ansiblue>'))
        ...
        if not servers:
            ...
            print_formatted_text(HTML('<ansiyellow>OpenHands will work but without MCP tools</ansiyellow>'))
            return True
        ...
        print_formatted_text(HTML('  • Test servers individually before adding multiple'))
    ...
    def interactive_troubleshooting(self):
        ...
        print_formatted_text(HTML('<ansiblue>🛠️ Interactive MCP Troubleshooting</ansiblue>'))
        print()
        
        while True:
            print_formatted_text(HTML('<ansiblue>What would you like to do?</ansiblue>'))
